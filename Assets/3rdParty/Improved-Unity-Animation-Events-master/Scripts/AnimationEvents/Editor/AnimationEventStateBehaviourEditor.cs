...
public class AnimationEventStateBehaviourEditor : Editor {
    ...
    AnimationEventStateBehaviour.AnimationEventData currentlySelectedEvent = new AnimationEventStateBehaviour.AnimationEventData();
    int currentlySelectedEventIndex = 0;
    public override void OnInspectorGUI() {
        ...
        if (Validate(stateBehaviour, out string errorMessage)) {
            ...
            currentlySelectedEventIndex = Math.Clamp(
                EditorGUILayout.IntField(new GUIContent("Preview event index"), currentlySelectedEventIndex), 
                0, 
                stateBehaviour.events.Count > 0 ? stateBehaviour.events.Count-1 : 0
            );
            currentlySelectedEvent = stateBehaviour.events[currentlySelectedEventIndex];
    ...
    void PreviewAnimationClip(AnimationEventStateBehaviour stateBehaviour) {
        ...
        if (motion is BlendTree blendTree) {
            SampleBlendTreeAnimation(stateBehaviour, currentlySelectedEvent.triggerTime);
            return;
        }

        // If it's a simple AnimationClip, sample it directly
        if (motion is AnimationClip clip) {
            previewTime = currentlySelectedEvent.triggerTime * clip.length;
    ...
    bool Validate(AnimationEventStateBehaviour stateBehaviour, out string errorMessage) {
        if (stateBehaviour.events.Count == 0)
        {
            errorMessage = "Add an event.";
            return false;
        }
